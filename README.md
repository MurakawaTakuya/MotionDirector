...
#### Option 2: Using venv (Recommended for Python 3.12+)
...
**Note**: The original `requirements.txt` contains specific versions that may not be compatible with Python 3.12+. The venv method above installs compatible versions of all required packages.

```bash
# いまの venv を有効化した状態で
pip install -U pip

# 1) cached_download がまだある版に固定
pip install "huggingface-hub==0.25.2"

# 2) Py3.12 でもホイールがある tokenizers に
